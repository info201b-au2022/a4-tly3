...
# what is the percentage of the state's population vs. percentage of jail +
#prison population for each group of people?
...
incarceration_pop_percent <- data %>%
  ...
  mutate(
    ...
    latinx_incarcerated_percent = latinx_incarcerated / total_incarcerated
    * 100,
    native_incarcerated_percent = native_incarcerated / total_incarcerated
    * 100,
...
state_most_incarcerated <- data %>%
  filter(year == 2016) %>%
  rowwise() %>%
  mutate(total = sum(total_jail_pop_rate, total_prison_pop_rate,
                     na.rm = TRUE)) %>%
  ungroup() %>%
  group_by(state) %>%
...
state_highest_women_incar <- data %>%
  filter(year == 2016) %>%
  group_by(state) %>%
...
plot_jail_pop_by_states <- function(states) {
  ...
  line_graph <- ggplot(to_plot, aes(x = year, y = total, color = state)) +
    labs(title = "Increase of Jail Population in the U.S. by State(s)
         (1970-2018)", x = "Year", y = "Total Jail Population",
...
get_state_v_jail_for_black_pop <- function() {
  ...
  state_v_incar_black_pop <-
    inner_join(state_black_pop_percent, incar_pop_percent_black, by = "state")
  return(state_v_incar_black_pop)
}
...
plot_state_v_jail_black_pop <- function() {
  to_plot <- get_state_v_jail_for_black_pop()
...
get_latinx_and_texas_data <- function() {
  texas_latinx_incar <- data %>%
    filter(year == 2016, state == "TX") %>%
    rowwise() %>%
    mutate(total_incar = sum(latinx_jail_pop, latinx_prison_pop,
                             na.rm = TRUE)) %>%
    select(state, county_name, total_incar)

  texas_latinx_incar$county_name <-
    sub(" .*", "", texas_latinx_incar$county_name)
